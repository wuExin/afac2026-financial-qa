...
requirements.txt 中只包含 baseline 运行必需的依赖（`openai` 和 `pyyaml`）。如果你需要重新解析 PDF，再额外安装 `pymupdf4llm`。
...
2. 调用 `pymupdf4llm` 把 86 份 PDF 逐页转成 Markdown，输出到 `data/processed_pymupdf4llm/`
